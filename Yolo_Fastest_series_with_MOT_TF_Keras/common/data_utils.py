...
def letterbox_resize(image, target_size, return_padding_info=False):
    ...
    image = image.resize(padding_size, Image.BILINEAR)
...
def random_resize_crop_pad(image, target_size, aspect_ratio_jitter=0.3, scale_jitter=0.5):
    ...
    rand_aspect_ratio = target_w/target_h * \
...
def random_rotate(image, boxes, rotate_range=20, prob=0.1):
    ...
    if warpAffine and rotate_range:
        ...
        if n:
            ...
            corner_points[:, :2] = boxes[:, [0, 1, 2, 3, 0, 3, 2, 1]].reshape(
                n * 4, 2)  # [xmin,ymin], [xmax,ymax], [xmin,ymax], [xmax,ymin]
            ...
            rotate_boxes = np.concatenate((corner_x.min(axis=-1), corner_y.min(
                axis=-1), corner_x.max(axis=-1), corner_y.max(axis=-1))).reshape(4, n).T
...
def random_gridmask(image, boxes, prob=0.2):
    ...
    # Returns
        ...
        boxes: rotated bounding box numpy array
    """
    grid = Grid(d1=image.size[0]//7, d2=image.size[0] //
                3, rotate=360, ratio=0.5, prob=prob)
...
def random_mosaic_augment(image_data, boxes_data, prob=.1):
    ...
    if not do_augment:
        ...
        for i in range(batch_size):
            ...
            crop_x = int(random.uniform(
                int(width*min_offset), int(width*(1-min_offset))))
            crop_y = int(random.uniform(int(height*min_offset),
                         int(height*(1 - min_offset))))

            merged_boxes = merge_mosaic_bboxes(
                bboxes, crop_x, crop_y, image_size=(height, width))
...
def random_cutmix_augment(image_data, boxes_data, prob=.1):
    ...
    if not do_augment:
        ...
        def get_cutmix_box(image_size, lamda):
            ...
            cut_xmin = np.clip(center_x - cut_w // 2,
                               int(width*min_offset), int(width*(1-min_offset)))
            cut_ymin = np.clip(
                center_y - cut_h // 2, int(height*min_offset), int(height*(1-min_offset)))
            cut_xmax = np.clip(center_x + cut_w // 2,
                               int(width*min_offset), int(width*(1-min_offset)))
            cut_ymax = np.clip(
                center_y + cut_h // 2, int(height*min_offset), int(height*(1-min_offset)))
        ...
        for i in range(batch_size):
            ...
            cut_xmin, cut_ymin, cut_xmax, cut_ymax = get_cutmix_box(
                image_size=(height, width), lamda=lamda)
            merged_boxes = merge_cutmix_bboxes(
                bboxes, cut_xmin, cut_ymin, cut_xmax, cut_ymax, image_size=(height, width))
            # no valid bboxes, drop this loop
            # if merged_boxes is None:
            #i = i - 1
            # continue
            ...
            bg_image[cut_ymin:cut_ymax, cut_xmin:cut_xmax,
                     :] = pad_image[cut_ymin:cut_ymax, cut_xmin:cut_xmax, :]
...
def preprocess_image(image, model_image_size, norm=True, grayscale=False):
    ...
    image = image.resize(model_image_size, Image.BILINEAR)
    ...
    # image_data = tf.image.resize(image, model_image_size)
