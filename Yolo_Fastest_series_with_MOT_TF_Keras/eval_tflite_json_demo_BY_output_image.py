...
def yolo_predict_tflite(interpreter, image, anchors, num_classes, conf_threshold, iou_threshold, elim_grid_sense,
                        v5_decode):
                ...
                for anc in range(3):
                    ...
                    if objectness > conf_threshold:
                        ...
                        for s in range(num_classes):
                            det['prob'][s] = sigmoid(
                                (output_data[h][w][anc * (num_classes + 5) + 5 + s] - zero_point) * scale) * objectness
...
def get_prediction_class_records(model, model_format, annotation_records, anchors, class_names, model_image_size, iou_threshold, conf_threshold, elim_grid_sense, v5_decode, save_result, output_image_path):
    '''
    ...
    for i, (image_name, gt_records) in enumerate(annotation_records.items()):
        ...
        for box, cls, score in zip(pred_boxes, pred_classes, pred_scores):
            ...
            if score > 0.5:
                ...
                draw.text((xmin+2, ymin+2), class_names[cls], fill=255, font=font)
                draw.rectangle([xmin, ymin, xmax, ymax], outline=(0, 0, 255), width=2)
        image.save(os.path.join(output_image_path, "{}.bmp".format(i)), "bmp")
        # image.show()
    pbar.close()
    return True
...
def eval_AP(model, model_format, annotation_lines, anchors, class_names, model_image_size, eval_type, iou_threshold,
            conf_threshold, elim_grid_sense, v5_decode, save_result, output_image_path, class_filter=None):
    '''
    ...
    result = get_prediction_class_records(model, model_format, annotation_records, anchors, class_names,
                                          model_image_size, iou_threshold, conf_threshold, elim_grid_sense, v5_decode,
                                          save_result, output_image_path)
    if result:
...
def main():
    ...
    # output_image_path = r"./output_image_2"
    # eval_type = "COCO"
    # iou_threshold = 0.45
    ...
    parser.add_argument('--output_image_path', type=str, required=False, default='./output_image_3',
                        help='the output image path of evaluate result , default=%(default)s')
    ...
    start = time.time()
    eval_AP(model, model_format, annotation_lines, anchors, class_names, model_image_size, eval_type, iou_threshold,
            conf_threshold, elim_grid_sense, v5_decode, save_result, output_image_path, class_filter=class_filter)
    end = time.time()
